...
for n in novos:
    ...
    if ex_emp:
        ...
        cname = ex_emp['company_name']
        ccid = ex_emp['cost_center_id']
        ccname = ex_emp['cost_center_name']
    else:
        # Check manual data
        if nome in manual_data:
            matricula, admissao = manual_data[nome]
            cid, cname, ccid, ccname = get_company_info("CONSTRUTORA ACPO")
        else:
            matricula = ""
            admissao = ""
            cid, cname, ccid, ccname = get_company_info("CONSTRUTORA ACPO") # Defaulting to matriz
            
    # Apply special name correction requested by user
    db_name = "CLAUDIA DE SOUZA MORGADO" if nome == "CLAUDIO DE SOUZA MORGADO" else nome
    
    emp_id = str(uuid.uuid4())
    
    nome_esc = db_name.replace("'", "''")
    mat_esc = matricula.replace("'", "''")
    adm_val = f"'{admissao}'" if admissao else "NULL"
    cid_val = f"'{cid}'" if cid else "NULL"
    ccid_val = f"'{ccid}'" if ccid else "NULL"
    
    sql = f"INSERT INTO employees (id, name, registration_number, admission_date, company_id, cost_center_id, status) VALUES ('{emp_id}', '{nome_esc}', '{mat_esc}', {adm_val}, {cid_val}, {ccid_val}, 'Ativo');"
    sql_statements.append(sql)

with open("insert_141_employees.sql", "w", encoding="utf-8") as f:
    f.write("\n".join(sql_statements))
