...
export const BUCKET_LABELS = {
  ...
  documentacao: "Documentação",
  mp: "Processo de MP",
  contratacao: "Contratados",
};

/**
 * Em qual balde o candidato cai. `encerrado` cobre Reprovado/Desistente, que não aparecem
 * na Central.
 *
 * Contratado NÃO é encerrado: fica na aba Contratação e continua lá. A aba já nasceu
 * contando zero sempre, porque todo desfecho caía em `encerrado` (issue #113); a correção de
 * lá deixava o contratado visível por 30 dias e depois o sumia sozinho. O prazo caiu: quem
 * foi contratado continua na Central.
 */
export function candidateBucket(status, etapaAtual) {
  if (sameStage(status, "Banco de Talentos")) return "livre";
  if (sameStage(status, "Contratado")) return "contratacao";
