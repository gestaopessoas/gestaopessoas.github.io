...
- **~~Status convivem em português e inglês.~~** Resolvido em 2026-09-04: a migration `20260904190000` normalizou as 4.505 linhas em `"inactive"` para `"Inativo"`, e `INACTIVE_STATUSES` / `ARCHIVE_STATUSES` deixaram de listar as duas grafias. Se `"inactive"` reaparecer, é dado novo entrando errado — investigue a origem em vez de recolocar na lista.
...
- **Três fontes para colaborador, e escolher errado é bug silencioso.** Depois da separação física (ADR 0009): `public.employees` = **só o quadro atual** (296); `public.employees_todos` = quadro atual + arquivo (4.839), é daqui que turnover, analytics, histórico e busca global leem; `public.arquivo_morto` = uma linha por **dossiê**, já com a caixa embutida. Quem saiu mora no schema `arquivo`.
- **Escrita vai por `employees_todos`, não por `employees`.** A view tem trigger `INSTEAD OF` que grava no schema certo. Salvar direto em `public.employees` a ficha de um arquivado afeta zero linhas e o formulário mostra erro. Vale o mesmo para `employee_archives_todos`.
- **O arquivamento é automático, todo dia às 03:00.** Um agendamento `pg_cron` (`arquivar-arquivo-morto`, `0 6 * * *` em UTC) move para o arquivo quem estiver em `public` com status de saída. Para conferir se está vivo: `select rotina_arquivamento_status()` — devolve a agenda, a última execução e quantos estão esperando. `esperando_arquivamento` acima de zero por mais de um dia significa que a rotina parou. O histórico de execuções está na view `arquivamentos`, e `e2e/rotina-arquivamento.spec.ts` falha se o agendamento sumir ou registrar erro.
- **Reativar é `reativar_colaborador(id)`** — traz a pessoa e o histórico de volta para `public` e deixa o dossiê na caixa. Arquivar à mão, fora da rotina, é `arquivar_colaboradores()`.
- **Join embutido do PostgREST não atravessa view com `UNION`.** Por isso `arquivo_morto` vem achatada e existe `physical_boxes_contagem`: `employee_archives(count)` e `physical_boxes(code)` não funcionam nessas views.
- **Alterou `employees`? Altere `arquivo.employees` também** e recrie as views `_todos`. A tabela espelho não acompanha sozinha.
- **Realtime está desligado de propósito.** As tabelas `employees`, `employee_benefits`, `rgs_processes` e `benefit_ignores` saíram da publicação `supabase_realtime` e voltaram a `REPLICA IDENTITY DEFAULT` (migration 20260904220000). Estavam publicadas com `REPLICA IDENTITY FULL`, o que fazia todo UPDATE gravar a linha antiga inteira no WAL para ninguém ler — o último `.channel()` do app saiu em 341f531. Se voltar a usar Realtime, republique as tabelas.
