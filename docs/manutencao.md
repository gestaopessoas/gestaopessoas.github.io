...
- **`arquivar_colaboradores()` precisa rodar de tempos em tempos.** Sem isso `public.employees` volta a acumular desligados e a separação apodrece. Reativar é `reativar_colaborador(id)` — traz a pessoa e o histórico de volta, e deixa o dossiê na caixa.
