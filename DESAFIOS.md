...
**Push na `main` já é deploy, e o CI aplica a migration antes.**
...
(PostgREST recusa o select) até a migration ser aplicada — por isso o job `migrate` roda
antes do build, e o `deploy` não publica se ele falhar. O job só acorda quando o push
mexe em `supabase/migrations/`, e espera aprovação no environment `banco-producao`.
Sem os segredos `SUPABASE_ACCESS_TOKEN` e `SUPABASE_DB_PASSWORD` cadastrados nesse
environment, push com migration falha no `link` e o deploy não sai.
